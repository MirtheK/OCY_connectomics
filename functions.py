...
def OCY_read_stack(mask):
    ...
    if(num_img>0):
        size = skimage.io.imread(files[0]).shape #size = cv2.imread(files[0], cv2.IMREAD_GRAYSCALE).shape
...
def OCY_thr_stack(img, THR):
    ...
    if np.median(img.reshape(-1, 1), axis=0) > 20:
        ...
        for i in range(0,num_img):
            img[:,:,i] = morphology.white_tophat(img[:,:,i], se) #cv2.morphologyEx(img[:,:,i], cv2.MORPH_TOPHAT, se) #
...
def OCY_fill_voids(bins, S):
    ...
    new = morphology.area_opening(new, 400)
...
def OCY_extract_cells(img, FOOTPRINT_SIZE=160, KERNEL_SIZE=5):
    ...
    coords = feature.peak_local_max(distance, footprint=np.ones((FOOTPRINT_SIZE, FOOTPRINT_SIZE, FOOTPRINT_SIZE)), labels=img)
